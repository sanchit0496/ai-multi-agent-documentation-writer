...
*   **Declarative:** Focuses on *what* needs to be achieved (e.g., "build this Docker image," "deploy to Kubernetes"). Examples include `.gitlab-ci.yml`, `.github/workflows/*.yml`, `azure-pipelines.yml`. These are generally easier to read and maintain for complex pipelines.
*   **Imperative:** Focuses on *how* to achieve it, specifying explicit steps and logic. `Jenkinsfile` (using Groovy DSL) can be imperative, offering greater flexibility but potentially more complexity.
