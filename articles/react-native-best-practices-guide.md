...
*   Libraries like `reselect` (for Redux) or direct selectors in Zustand memoize their results. This means a component subscribing to a selector will only re-render if the *derived value* from the selector actually changes, not just if the overall state tree changes reference.
