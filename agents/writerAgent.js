...
export const executeWriterPhase = async (state) => {
  ...
  logger.info(
    'WriterPhase',
    'Starting production article generation.'
  );

  if (!state.strategy) {
    throw new Error('WriterPhase Error: Missing required upstream data "state.strategy"');
  }

  if (!state.researchData) {
    throw new Error('WriterPhase Error: Missing required upstream data "state.researchData"');
  }

  if (!state.outline) {
    throw new Error('WriterPhase Error: Missing required upstream data "state.outline"');
  }
  ...
  try {
    ...
    logger.error(
      'WriterPhase',
      'Failed to generate article draft.',
      error
    );

    throw new Error(
      `WriterPhase Critical Failure: ${error.message}`
    );
