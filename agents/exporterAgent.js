...
export const executeExporterPhase = async (state) => {
  ...
  try {
    ...
    logger.info('ExporterPhase', 'Article exported successfully.', {
      markdownFile: markdownFilePath,
      seoFile: seoFilePath,
    });

    return { ...state, finalMarkdownFile: markdownFilePath };
  } catch (error) {
    logger.error('ExporterPhase', 'Failed to export article.', error);
    throw new Error(`ExporterPhase Critical Failure: ${error.message}`);
  }
};
